...
local a_b = ui_new_checkbox("AA", "Anti-aimbot angles", "KRIP YAW")
local visuals = {
...
local KRIPSI_ROLL = ui_new_checkbox("AA", "Anti-aimbot angles", "KRIP ROLL")
...
local enable_adfucker = ui.new_hotkey("AA", "Anti-aimbot angles", "KRIP AS (AIR-SPAM)")
...
client.color_log(21, 235, 220,  "                         KRIP YAW  INJECTED       ")
client.color_log(215, 115, 222, "              (︶︹︺) Coded by KRIPSI#5061 (︶︹︺)               ")
client.color_log(235, 221, 21,  "                    Last Updated: 20/07/2022.               ")
client.color_log(255, 255, 255, "|--------------------------------------------------------|")
...
local function on_paint(ctx)
	if (ui_get(a_b)) then
		client.draw_indicator(c, 124, 252, 252, 252, "KRIP YAW") 
	end
...
local lua_log = function(...) --inspired by sapphyrus' multicolorlog
    client.color_log(255, 59, 59, "[ KRIP ROLL ]\0")
    local arg_index = 1
...
local function indicators()
    ...
    local ind_size = { renderer_measure_text("cb", "KRIP YAW") };
    ...
    if ui.get(visuals.ind_select) == "Second" then
        ...
        renderer.text(cx, cy + 50, 255, 255, 255, 255 * global_alpha, 'c', 0, 'KRIP YAW')
    end
end
